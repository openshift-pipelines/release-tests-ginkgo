...
func getSubcription(cs *clients.Clients, name string) (*v1alpha1.Subscription, error) {
	subscription, err := cs.OLM.OperatorsV1alpha1().Subscriptions(OperatorsNamespace).Get(context.Background(), name, metav1.GetOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to get subscription %s in namespace %s: %w", name, OperatorsNamespace, err)
	}
	return subscription, nil
}

func createSubscription(name, channel, catalogsource string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to read subscription template: %w", err)
	}

	sub, err := template.New("subscription").Parse(string(tmpl))
	if err != nil {
		return fmt.Errorf("failed to parse subscription template: %w", err)
	}

	var buffer bytes.Buffer
	if err = sub.Execute(&buffer, subscription); err != nil {
		return fmt.Errorf("failed to execute subscription template: %w", err)
	}
...
func OperatorCleanup(cs *clients.Clients, name string) error {
	sub, err := getSubcription(cs, name)
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("failed to delete CSVs in namespace %s: %w", OperatorsNamespace, err)
	}
